...
import com.craig.recipebook.config.KafkaProperties;
...
class RecipebookKafkaResourceIT {
    ...
    void setup() {
        ...
        RecipebookKafkaResource kafkaResource = new RecipebookKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/recipebook-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/recipebook-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
